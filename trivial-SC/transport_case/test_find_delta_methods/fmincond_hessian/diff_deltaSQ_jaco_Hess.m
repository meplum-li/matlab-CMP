function [diff_delta, J, Hess] = diff_deltaSQ_jaco_Hess(delta0)
...
temdiff_delta = real(delta_new_i - delta0);
diff_delta = temdiff_delta^2;
if nargout > 1
    temJ = real(mean(-Ui./(2*pi*1i).*int(:,2)) - 1);
J = 2*temdiff_delta*temJ;
end
if nargout >2
    temHess = real(mean(-Ui./(2*pi*1i).*int(:,3)));
Hess = 2*temJ*temJ + 2*temdiff_delta*temHess;
end
%%
