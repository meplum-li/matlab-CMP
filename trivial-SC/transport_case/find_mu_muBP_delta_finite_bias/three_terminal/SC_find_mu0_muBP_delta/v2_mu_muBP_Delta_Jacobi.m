function [F, J] = v2_mu_muBP_Delta_Jacobi(Sample, x)
mu0 = x(1);
mu_BP = x(2);%A_mu_exBP
delta = x(3);
Ui = Sample.Ui;
...
int=integral(@(EF) v2_current_diffDelta_jacobi(Sample, EF, mu0, mu_BP, delta),-inf,inf,"ArrayValued",true,'RelTol',gap_RelTol,'AbsTol',gap_AbsTol);
fprintf('%6.2E  %6.2E  %6.2E\n',int(1:3))
F(1) = sum(int(1:3));
F(2) = int(3);
A=Ui/(2*pi)*int(4) - delta;
F(3) = A*conj(A);
...
J(3,1) = 2*real(J(3,1)*conj(A));
J(3,2) = Ui/(2*pi)*(-1) * sum([1;-1].*[BPe_GRGA41;BPh_GRGA41]);%DF(3)/Dmu_BP
J(3,2) = 2*real(J(3,2)*conj(A));
J(3,3) = Ui/(2*pi)*int(12) - 1;%DF(3)/Ddelta
J(3,3) = 2*real(J(3,3)*conj(A));
