function [F, J] = v3_mu_muBP_Delta_Jacobi(Sample, x)
mu0 = x(1);
mu_BP = x(2);%A_mu_exBP
delta = x(3);
Ui = Sample.Ui;
...
int=integral(@(EF) v3_current_diffDelta_jacobi(Sample, EF, mu0, mu_BP, delta),-inf,inf,"ArrayValued",true,'RelTol',gap_RelTol,'AbsTol',gap_AbsTol);
fprintf('%6.2E  %6.2E  %6.2E\n',int(1:3))
F(1) = sum(int(1:3));
F(2) = int(3);
diffDelta=Ui/(2*pi)*int(4) - delta;
F(3) = real(diffDelta);
F(4) = imag(diffDelta);
...
diffDelta_Dmu0 = Ui/(2*pi)*int(11) +Ui/(2*pi)*sum( kron(ones(3,1),[1;-1]).*[Ue_GRGA41;Uh_GRGA41;De_GRGA41;Dh_GRGA41;BPe_GRGA41;BPh_GRGA41] );%DF(3)/Dmu0
J(3,1) = real(diffDelta_Dmu0);
J(4,1) = imag(diffDelta_Dmu0);
diffDelta_DmuBP = Ui/(2*pi)*(-1) * sum([1;-1].*[BPe_GRGA41;BPh_GRGA41]);%DF(3)/Dmu_BP
J(3,2) = real(diffDelta_DmuBP);
J(4,2) = imag(diffDelta_DmuBP);
diffDelta_Ddelta = Ui/(2*pi)*int(12) - 1;%DF(3)/Ddelta
J(3,3) = real(diffDelta_Ddelta);
J(4,3) = imag(diffDelta_Ddelta);
