function [F, J] = current_mu_muBP_Delta_Jacobi(Sample, x)
mu0 = x(1);
mu_BP = x(2);%A_mu_exBP
delta = x(3);
current_RelTol = Sample.current.RelTol;
current_AbsTol = Sample.current.AbsTol;
int_current=integral(@(EF) i_E(Sample, EF, mu0, mu_BP, delta),-inf,inf,"ArrayValued",true,'RelTol',current_RelTol,'AbsTol',current_AbsTol);
% ub = max(abs(Sample.A_mu_exU-mu0),abs(Sample.A_mu_exD-mu0)); 
% int_current=integral(@(EF) i_E(Sample, EF, mu0, mu_BP, delta),-ub,ub,"ArrayValued",true,'RelTol',current_RelTol,'AbsTol',current_AbsTol);
fprintf('%6.2E  %6.2E  %6.2E\n',int_current(1:3))
% display(int(1:3))
F(1)=sum(int_current(1:3));
F(2)=int_current(3);
%%
TT_Ue = transmission(Sample, Sample.A_mu_exU-mu0 , mu0, mu_BP, delta);
TT_Uh = transmission(Sample, -Sample.A_mu_exU+mu0 , mu0, mu_BP, delta);
TT_De = transmission(Sample, Sample.A_mu_exD-mu0 , mu0,mu_BP, delta);
TT_Dh = transmission(Sample, -Sample.A_mu_exD+mu0 , mu0,mu_BP, delta);
TT_BPe = transmission(Sample, mu_BP-mu0 , mu0, mu_BP, delta);
TT_BPh = transmission(Sample, -mu_BP+mu0 , mu0, mu_BP, delta);
...
J(1,1) = sum(int_current(4:6))+Dmu2_1+Dmu2_2;%DF(1)/Dmu0
J(1,2) = sum([TT_BPe(5,:),TT_BPh(6,:)])-sum(sum( kron(ones(3,1),[1,-1;-1,1]).*[TT_BPe(:,5),TT_BPh(:,6)] ));%DF(1)/Dmu_BP
J(2,1) = int_current(6) -sum( [TT_BPe(5,:),TT_BPh(6,:)] ) + sum(sum( kron(ones(1,3),[1,-1;-1,1]).*[TT_Ue([5,6],1),TT_Uh([5,6],2),TT_De([5,6],3),TT_Dh([5,6],4),TT_BPe([5,6],5),TT_BPh([5,6],6)] ));%DF(2)/Dmu0
J(2,2) = sum([TT_BPe(5,:),TT_BPh(6,:)])-sum(sum( [1,-1;-1,1].*[TT_BPe([5,6],5),TT_BPh([5,6],6)] ));
%%
    function [i_vec] = i_E(Sample, EF, mu0, A_mu_exBP, delta)
        h = Sample.h;
...
%%
    function [trans] = transmission(Sample, EF, mu0, A_mu_exBP, delta)
        h = Sample.h;
        alphaR = Sample.alphaR;
        periodcity = Sample.periodicity;
        sigma0 = eye(2);
